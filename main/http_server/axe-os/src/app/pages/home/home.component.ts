...
export class HomeComponent implements AfterViewChecked, OnInit, OnDestroy {
   ...
   *
   * Priority:
   * 1) Use an existing current field if the backend already provides it
   * 2) Fallback: compute from Power (W) / Input Voltage (V)
   */
  get inputCurrentA(): number {
    // Try to find a "stats-like" object commonly used on the home dashboard
    const src: any =
      (this as any).stats ??
      (this as any).status ??
      (this as any).systemInfo ??
      (this as any).miner ??
      (this as any).data ??
      this;

    // 1) Prefer an existing current reading if present (already in A or mA)
    const candidatesA = [
      src?.current,
      src?.inputCurrent,
      src?.input_current,
    ...
    // If any candidate looks like a sane Ampere reading, use it.
    for (const v of candidatesA) {
      const n = Number(v);
      if (Number.isFinite(n) && n > 0 && n < 200) return n; // 200A guardrail (way above realistic)
    }

    // Some firmwares expose current in mA
    const candidatesmA = [
      src?.current_mA,
      src?.current_ma,
      src?.inputCurrentmA,
      src?.input_current_ma,
      src?.vinCurrentmA,
      src?.vin_current_ma,
    ];

    for (const v of candidatesmA) {
      const n = Number(v);
      if (Number.isFinite(n) && n > 0 && n < 200000) return n / 1000.0; // mA -> A
    }

    // 2) Fallback: compute current from power/voltage
    const powerW =
      Number(src?.power ?? src?.power_w ?? src?.powerW ?? src?.watts ?? 0);
    ...
    if (!Number.isFinite(powerW) || !Number.isFinite(vinV)) return 0;
    if (vinV <= 0.1) return 0; // avoid divide-by-zero/bogus readings
